...
//
// 管理者専用 — estimate_requests のステータスを安全に更新する。
...
// 許可するステータス値のみ
const ALLOWED_STATUSES = ['new', 'in_progress', 'done', 'hidden'] as const;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const url = `${SUPABASE_URL}/rest/v1/estimate_requests?id=eq.${encodeURIComponent(id)}&select=id,status`;
